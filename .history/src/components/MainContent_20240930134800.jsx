...
function MainContent() {
  ...
  return (
    <div  className='max-w-[1251px] flex flex-col  py-32'>
      <CountryList setCountries={setCountries}/>
      <div className='ArabCountries gr  grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-20'>
      {countries.map((country) => (
       ArabCountries.includes(country.name.common.trim())   ? <CountryCard key={country.name.common} country={country} /> : null
      ))}
      </div>
      <div className='forginCountires grid grid-cols-4'>
      {countries.map((country) => (
       !ArabCountries.includes(country.name.common.trim())   ? <CountryCard key={country.name.common} country={country} /> : null
      ))}
      </div>
